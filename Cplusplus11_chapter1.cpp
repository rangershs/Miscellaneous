...
/*
 *	auto推导的是class的value_type，而不是迭代器的类型；若不使用&，那么将会拷贝出元素的副本
 *	基于范围的for循环中遍历map和set，无法修改关联容器的key值，因为它们的key是只读的(set中的key就是value),auto&会被推导为const auto&
 *	基于范围的for循环，冒号后面的表达式只会执行一次，与循环的次数无关
